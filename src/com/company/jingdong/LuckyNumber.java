...
/**
 * 4和7是两个幸运数字，我们定义，十进制表示中，每一位只有4和7两个数的正整数都是幸运数字。前几个幸运数字为：4,7,44,47,74,77,
 * 444,447...现在输入一个数字K，输出第K个幸运数。
 * <p>
 * 输入
 ...
 * 于是第k个幸运数就是从位置为k的叶子结点向上遍历直到根节点的左或右子节点的路径。根据完全二叉树的性质，可以由子节点的位置求
 * 得其父节点的位置。若子节点是奇数位置n，则其父节点是(n-1)/2；若子节点是偶数位置，则其父节点是(n-2)/2。遍历过程中拼接字符串
 * 即可得到幸运数。
 *
 * @author XIAO
 */
