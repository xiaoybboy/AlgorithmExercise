 ...
 * 而dp[i][j]的值则表示当A-B的值为j时，B的最大长度是多少。 dp[i][j] = dp[i-1][j]表示我不用第i块砖 dp[i][j] =
 * max(dp[i-1][j-h], dp[i-1][j])表示我把砖放在A堆。 dp[i][j] = max(dp[i-1][j+h]+h,
 * dp[i-1][j])表示我把砖放在B堆。 然后会爆内存，所以用了滚动数组。
 *
 * @author XIAO
 */
public class Brick {
